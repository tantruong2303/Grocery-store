...
const createEntity = (name: string, quantity: number, price: number, id: string) => {
...
paymentList.forEach((radio) => {
        radio.addEventListener("click", function () {
                payment = Number((radio as HTMLInputElement).value);
        });
});

const order = document.getElementById("order");
order?.addEventListener("submit", function (event: Event) {
